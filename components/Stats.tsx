...
export default function Stats() {
  ...
  return (
    <section ref={ref} style={{ position: "relative", padding: "clamp(60px, 10vw, 120px) 0", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: "1280px", margin: "0 auto", padding: "0 clamp(20px, 5vw, 40px)" }}>
        <div style={{ textAlign: "center", marginBottom: "clamp(48px, 7vw, 80px)" }}>
          ...
          <h2 style={{ fontSize: "clamp(28px, 4vw, 56px)", fontWeight: 900, letterSpacing: "-0.03em", color: "#FFFFFF", lineHeight: 1.1 }}>
            {t("title")}{" "}
        ...
        <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "2px" }}>
          {items.map((s, i) => (
            <div key={s.label} style={{ textAlign: "center", padding: "clamp(28px, 4vw, 48px) clamp(16px, 3vw, 32px)", background: i === 0 ? "rgba(206,86,5,0.15)" : "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(24px)", transition: "all 0.6s ease " + (i * 0.1) + "s" }}>
              <div style={{ fontSize: "clamp(40px, 5vw, 80px)", fontWeight: 900, color: "#CE5605", letterSpacing: "-0.03em", lineHeight: 1, marginBottom: "4px" }}>
                {s.value}<span style={{ color: "#EA7628" }}>{s.suffix}</span>
              </div>
              <div style={{ fontSize: "clamp(11px, 1.5vw, 15px)", fontWeight: 800, color: "#FFFFFF", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "8px" }}>{s.label}</div>
